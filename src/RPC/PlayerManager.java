...
public class PlayerManager extends UnicastRemoteObject implements RPCGameInterface{
	...
	public GameManager gameManager = new GameManager(); 
	 ...
	 public boolean startConnectionToOtherPlayer(){
	    	try {	
	    		this.guestRPCConnection = thisPlayer == 1? (RPCGameInterface)Naming.lookup(PlayerManager.HOST2) : (RPCGameInterface)Naming.lookup(PlayerManager.HOST1);
	    		return true;
	    	} catch (Exception e){
	...
	public void setPlayerAction(String choosedCell, int player) throws RemoteException {
		...
		if(guestLastPlay == "") {
			guestLastPlay = choosedCell+player;
		}
