...
public class GameView {
	...
	private boolean shouldListenOneLastTime = false;
	...
	private void initialize() {
		...
		giveUpButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					sendTextToChatLocally("[SISTEMA] VOCÊ DESISTIU");
					player.guestRPCConnection.sendSystemMessage("ADVERSÁRIO DESISTIU, VOCÊ VENCEU");
					player.actualPlayer = 0;
					player.guestRPCConnection.setActualPlayer(player.actualPlayer);
					changeButtonsHabilitation(false);
	...
	private void changeButtonsHabilitation(boolean habilitation) {
		allButtons.forEach(button -> {
			button.setEnabled(habilitation);
		});
	}
		...
		(new SwingWorker<Void, Void>() {
			...
			public Void doInBackground() {
				while (player.isGameOn) {
					player.isGameOn = !shouldListenOneLastTime;
					try {
						Thread.sleep(500);
						if (player.guestLastMessage != "") {
							String currentText = chat.getText();
							chat.setText(currentText.equals("") ? player.guestLastMessage
			...
			public Void doInBackground() {
				...
				if (asPlayer1) {
					if (firstPlayer == 1) {
						...
						sendTextToChatLocally("[SISTEMA] VOCÊ COMEÇA");
						try {
						...
						sendTextToChatLocally("[SISTEMA] ADVERSÁRIO COMEÇA");
						try {
